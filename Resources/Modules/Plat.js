...
Plat.prototype.plotInScene = function( xi, yi, gi ) {

    var objx = this.parts[gi];  // interrogate parts factory
    if ( objx !== null ) {
        ...
        if ( (layerx !== null && layerx.name == "Tile Layer 1") || tileMap.numLayers == 1 ) { // version 1 ONLY reads this layer name
            ...
            for ( xx=0; xx<layerx.height; xx++) {  // now unpack the tmx map
                for ( yy=0; yy<layerx.width; yy++ ) {
                    ...
                    this.plotInScene ( (layerx.height-1)-xx, yy, mygid ); // make it look like it is in tiled
                }
            }
        }
    }
